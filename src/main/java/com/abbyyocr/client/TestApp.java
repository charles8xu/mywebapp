...
public class TestApp {
    ...
    public static void main(String[] args) {
        //在程序内初始化
        args = new String[]{"textField","/home/xuchao/pictures/20140923.jpg","/home/xuchao/pictures/result.xml"};
        ...
        //可以使用地址'https://cloud.ocrsdk.com'确保安全连接
     ...
     */
    private static void performRecognition(List<String> argList)
            throws Exception {
        String language = CmdLineOptions.extractRecognitionLanguage(argList);
        String outputPath = argList.get(argList.size()-1);
        ...
        if (argList.size() == 1) {
            ...
            task = restClient.processImage(argList.get(0), settings);
            ...
            for (int i = 0; i < argList.size(); i++) {
                ...
                Task result = restClient.submitImage(argList.get(i),
                        taskId);
                if (task == null) {
                    task = result;
                }
     ...
     * vCard, csv or xml
     */
    private static void performBusinessCardRecognition(List<String> argList)
            throws Exception {
        String language = CmdLineOptions.extractRecognitionLanguage(argList);
        String outputPath = argList.get(argList.size()-1);
        argList.remove(argList.size() - 1);
        ...
        Task task = restClient.processBusinessCard(argList.get(0),
                settings);
        waitAndDownloadResult(task, outputPath);
    }
    ...
    private static void performTextFieldRecognition(List<String> argList)
            throws Exception {
        String language = CmdLineOptions.extractRecognitionLanguage(argList);
        ...
        System.out.println("Uploading..");
        Task task = restClient.processTextField(argList.get(0), settings);
    ...
    private static void performBarcodeRecognition(List<String> argList)
            throws Exception {
        String outputPath = argList.get(argList.size()-1);
        argList.remove(argList.size() - 1);
        // argList now contains list of source images to process
        ...
        System.out.println("Uploading..");
        Task task = restClient.processBarcodeField(argList.get(0),
                settings);
     ...
     */
    private static void performFieldsRecognition(List<String> argList)
            throws Exception {
        ...
        String settingsPath = argList.get(argList.size()-1);
        argList.remove(argList.size() - 1);
        // argList now contains list of source images to process
        ...
        for (int i = 0; i < argList.size(); i++) {
            ...
            Task result = restClient.submitImage(argList.get(i), taskId);
            if (task == null) {
                task = result;
            }
        }
    ...
    private static void performMrzRecognition(List<String> argList)
            throws Exception {
        String outputPath = argList.get(argList.size()-1);
        argList.remove(argList.size() - 1);
        // argList now contains list of source images to process
        ...
        System.out.println("Uploading..");
        Task task = restClient.processMrz(argList.get(0));
    ...
    private static void performCaptureData(List<String> argList)
            throws Exception {
        String outputPath = argList.get(argList.size()-1);
        argList.remove(argList.size() - 1);

        String templateName = argList.get(argList.size()-1);
        argList.remove(argList.size() - 1);
        ...
        System.out.println("Uploading..");
        Task task = restClient.captureData(argList.get(0), templateName);
    ...
    private static void createTemplate(List<String> argList) throws Exception {
        if (argList.size() != 3) {
            System.out.println("Invalid number of arguments.");
            return;
        }

        String imagePath = argList.get(0);
        String templateName = argList.get(1);
        String settingsPath = argList.get(2);
     ...
     */
    private static String extractExtraOptions(List<String> args) {
